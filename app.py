...
# Cache pour stocker les hash des derniers uploads et éviter les uploads redondants
if "last_upload_hashes" not in st.session_state:
    st.session_state.last_upload_hashes = {}
if "last_upload_time" not in st.session_state:
    st.session_state.last_upload_time = {}


def upload_db_to_github(filepath, repo_filename, force=False):
    """
    Upload une base de données vers GitHub avec optimisations :
    - Détection de changements par hash local
    - Cache des hash pour éviter reuploads inutiles
    - Pas de debounce temporel pour éviter perte de données
    """
    if not GITHUB_TOKEN:
    ...
    # Calcul du hash local pour détection de changements RÉELS
    current_hash = _sha256_file(path_to_read)
    
    # Vérification dans le cache : si le hash est identique au dernier upload, skip
    # Cela évite les uploads redondants sans risquer de perdre des données
    if not force and repo_filename in st.session_state.last_upload_hashes:
        if st.session_state.last_upload_hashes[repo_filename] == current_hash:
            return False  # Pas de changement réel, skip silencieux
    ...
    if get_resp.status_code == 200:
        ...
        if data.get("encoding") == "base64" and data.get("content"):
            if data["content"].strip() == local_b64.strip():
                # Mise à jour du cache même si on skip
                st.session_state.last_upload_hashes[repo_filename] = current_hash
                return False
    ...
    put_resp = requests.put(f"{GITHUB_API_URL}/{repo_filename}", headers=headers, json=data_put, timeout=30)
    if put_resp.status_code >= 400:
        ...
        # Mise à jour du cache après upload réussi
        st.session_state.last_upload_hashes[repo_filename] = current_hash
        st.toast(f"📤 {repo_filename} envoyé sur GitHub ✅")
        return True

def upload_databases_batch():
    """
    Upload les deux bases de données en batch pour réduire le temps total.
    Optimisation : upload seulement si changements détectés.
    """
    uploaded_count = 0
    ...
    if uploaded_count > 0:
        st.success(f"✅ {uploaded_count} base(s) de données synchronisée(s)")
...
def create_triggers_for_table(cursor, conn, table_name):
    """
    Crée les triggers pour une table spécifique.
    Optimisation : permet de créer les triggers uniquement pour une nouvelle table
    au lieu de recréer tous les triggers à chaque fois.
    """
    trig_ins = f"trg_{table_name}_after_insert"
    trig_upd = f"trg_{table_name}_after_update"
    ...
    AFTER UPDATE OF dernier_avis, delai ON {table_name}
    FOR EACH ROW
    WHEN NEW.dernier_avis IS NOT NULL
    BEGIN
        UPDATE {table_name}
...
def create_triggers_for_fiches(cursor, conn, category_map):
    """
    Crée les triggers pour toutes les tables de fiches.
    Utilisé au démarrage de l'application pour s'assurer que tous les triggers existent.
    """
    for _, table in category_map.items():
        create_triggers_for_table(cursor, conn, table)
    ...
    with st.expander("➕ Ajouter ou supprimer une catégorie"):
        ...
        if st.button("Créer la catégorie", key="create_cat"):
            if new_cat:
                ...
                # Créer les triggers uniquement pour la nouvelle table (optimisation)
                create_triggers_for_table(cursor_fiches, conn_fiches, table_fiche)
                
                # Upload batch optimisé (détection de changements + debounce automatique)
                upload_databases_batch()
                
                st.success(f"✅ Catégorie '{new_cat}' ajoutée avec succès.")
                # Le rerun va automatiquement recharger category_map et avis_table_map
                st.rerun()
